...
import { widoApi } from './zap-providers/wido/widoApi'
...
export enum WidoZapType {
  ZAP_WIDO = 6
}

export type MergedZapType = ZapType | WidoZapType
...
const zapSlice = createSlice({
  ...
  reducers: {
    ...
    setOutputCurrency: (state, action: PayloadAction<{ currency1: string; currency2: string, outputProvider: string | null }>) => {
      state[Field.OUTPUT_APE_ZAP_UNIV2] = { currency1: action.payload.currency1, currency2: action.payload.currency2 }
      state[Field.OUTPUT_WIDO] = { toAddress: action.payload.outputProvider }
    },
...
export const zapProtocolReducers = combineReducers({
  [widoApi.reducerPath]: widoApi.reducer,
  // FIXME:
...
export function isNativeCurrencyId(currencyId: string | null | undefined): boolean {
