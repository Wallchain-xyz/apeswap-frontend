...
const Services: React.FC<{ bab?: boolean }> = ({ bab }) => {
  ...
  const slideNewsNav = (index: number) => {
    ...
    swiper?.slideTo(displayData ? displayData?.length + index : 0)
    swiper?.autoplay.start()
  }

  const handleSlide = (event: SwiperCore) => {
    const slideNumber = getDotPos(event.activeIndex, displayData ? displayData.length : 0)
                  ...
                  <Flex sx={{ paddingLeft: '15px', justifyContent: 'center', flexDirection: 'column' }}>
                    ...
                    {id === 'lendingDetails' ? (
                      ...
                      <Text sx={{ width: '100%', color: 'primaryBright', fontWeight: 400 }}>
                        Discount:{' '}
                        <span
                          sx={{ color: stat?.discount ? (stat.discount > 0 ? 'white' : '#DF4141') : 'transparent' }}
                        >
                      ...
                      <Text sx={{ width: '100%', color: 'primaryBright', fontWeight: 400 }}>
                        APR: {(stat?.apr ? stat.apr * 100 : 0).toFixed(2)}%
                      </Text>
                    )}
                  </Flex>
                </Flex>
